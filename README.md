...
would be:
``` cmake
git clone --recursive https://github.com/MattClarkson/scikit-surgeryopencvcpp
mkdir scikit-surgeryopencvcpp-Build
cd scikit-surgeryopencvcpp-Build
ccmake ../scikit-surgeryopencvcpp
make
```
...
```StartVS_Debug.bat``` or ```StartVS_Release.bat``` in the ```SKSURGERYOPENCVCPP-build``` folder.
...
* Clone scikit-surgeryopencvcpp, using ```--recursive```.
...
```
import sksurgeryopencvpython as mp
