...
//------------------------------------------------------------------------------
static void ValidateImages(const cv::Mat& leftImage, const cv::Mat& rightImage)
{
...
//------------------------------------------------------------------------------
static cv::Ptr<cv::stereo::QuasiDenseStereo> DoStereoMatching(
  const cv::Mat& leftImage,
  const cv::Mat& rightImage)
{
  ValidateImages(leftImage, rightImage);
  ...
  )
{
  ...
  cv::Ptr<cv::stereo::QuasiDenseStereo> stereo = DoStereoMatching(leftImage, rightImage);
  ...
  )
{
  ...
  cv::Mat matchedPoints = cv::Mat(static_cast<int>(matches.size()), 4, CV_64FC1);
  for (size_t i = 0; i < matches.size(); i++)
  {
    matchedPoints.at<double>(static_cast<int>(i), 0) = matches[i].p0.x;
    matchedPoints.at<double>(static_cast<int>(i), 1) = matches[i].p0.y;
    matchedPoints.at<double>(static_cast<int>(i), 2) = matches[i].p1.x;
    matchedPoints.at<double>(static_cast<int>(i), 3) = matches[i].p1.y;
  }
