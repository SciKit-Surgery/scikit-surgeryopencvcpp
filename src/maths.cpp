...
//-----------------------------------------------------------------------------
static double DistanceToLine(const std::pair<cv::Point3d, cv::Point3d>& line,
                             const cv::Point3d& x0)
{
  cv::Point3d x1 = line.first;
  cv::Point3d x2 = line.second;
                            ...
                            cv::Point3d& midpoint)
{
  ...
  if (std::isnan(sc) || std::isnan(tc) || std::isinf(sc) || std::isinf(tc))
  {
    distance = DistanceToLine(std::pair<cv::Point3d, cv::Point3d>(P0, P0 + u), Q0);
...
double ComputeRMSBetweenCorrespondingPoints(const cv::Mat& a, const cv::Mat& b)
{
  ...
  for (int r = 0; r < a.rows; r++)
  {
    ...
    {
      double diff = b.at<double>(r, c) - a.at<double>(r, c);
      rms += diff * diff;
    }
  }
