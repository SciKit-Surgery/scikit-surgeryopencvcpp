...
TEST_CASE("Empty points throws exception", "[triangulate]")
{
  REQUIRE_THROWS(sks::TriangulatePointsUsingHartley(
    cv::Mat(0, 4, CV_64FC1),
    cv::Mat(3, 3, CV_64FC1),
    cv::Mat(3, 3, CV_64FC1),
    cv::Mat(3, 3, CV_64FC1),
  ...
  REQUIRE_THROWS(sks::TriangulatePointsUsingMidpointOfShortestDistance(
    cv::Mat(0, 4, CV_64FC1),
    cv::Mat(3, 3, CV_64FC1),
    cv::Mat(3, 3, CV_64FC1),
    cv::Mat(3, 3, CV_64FC1),
...
TEST_CASE("Invalid matrix sizes throw exceptions", "[triangulate]")
{
  // Wrong left camera rows
  REQUIRE_THROWS(sks::TriangulatePointsUsingHartley(
    cv::Mat(1, 4, CV_64FC1),
    cv::Mat(2, 3, CV_64FC1),
    cv::Mat(3, 3, CV_64FC1),
...
TEST_CASE("Chessboard triangulation test", "[triangulate]")
{
  ...
  cv::Mat leftTranslation = cv::Mat(3, 1, CV_64FC1);
  ...
  cv::Mat leftToRightTranslation = cv::Mat(3, 1, CV_64FC1);
  ...
  cv::Mat modelPoints = cv::Mat::zeros(4, 3, CV_64FC1);
  modelPoints.at<double>(0, 0) = 0;  modelPoints.at<double>(0, 1) = 0;  modelPoints.at<double>(0, 2) = 0;
  modelPoints.at<double>(1, 0) = 39; modelPoints.at<double>(1, 1) = 0;  modelPoints.at<double>(1, 2) = 0;
  modelPoints.at<double>(2, 0) = 0;  modelPoints.at<double>(2, 1) = 27; modelPoints.at<double>(2, 2) = 0;
  modelPoints.at<double>(3, 0) = 39; modelPoints.at<double>(3, 1) = 27; modelPoints.at<double>(3, 2) = 0;

  cv::Mat pointsFromHartley = sks::TriangulatePointsUsingHartley(
    pointsIn2D, leftIntrinsic, rightIntrinsic, leftToRightRotation, leftToRightTranslation);

  cv::Mat pointsFromMidpoint = sks::TriangulatePointsUsingMidpointOfShortestDistance(
    pointsIn2D, leftIntrinsic, rightIntrinsic, leftToRightRotation, leftToRightTranslation);

  // Transform model points to left camera space for comparison
  cv::Mat modelPointsTransposed;
  cv::transpose(modelPoints, modelPointsTransposed);

  cv::Mat rotatedModelPoints;
  ...
  cv::Mat transformedModelPoints = cv::Mat(modelPointsRotatedAndTransposed.rows,
                                           modelPointsRotatedAndTransposed.cols, CV_64FC1);
  for (int r = 0; r < modelPointsRotatedAndTransposed.rows; r++)
  {
    for (int c = 0; c < modelPointsRotatedAndTransposed.cols; c++)
    {
      transformedModelPoints.at<double>(r, c) =
        modelPointsRotatedAndTransposed.at<double>(r, c) + leftTranslation.at<double>(c, 0);
    }
  }
